...
Copy-Item -LiteralPath (Join-Path $projectDir 'src-tauri\target\release\ColdDrop.exe') -Destination (Join-Path $projectDir 'artifacts\ColdDrop-1.1.0-portable.exe')
...
Copy-Item -LiteralPath $installer.FullName -Destination (Join-Path $projectDir 'artifacts\ColdDrop-1.1.0-setup.exe')
